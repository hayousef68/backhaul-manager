...
# ====================================================================
#
#          🚀 Backhaul Manager v3.2 (Final Stable Menu) 🚀
#
#   This version directly integrates the menu handling and color
#   functions from the user-provided stable script sample to
#   definitively resolve flickering and input issues.
#
# ====================================================================
...
NC='\033[0m' # No Color in case colorize fails


# --- UI & Helper Functions (from rathole_v2.sh) ---
colorize() {
    local color="$1"
    local text="$2"
...
check_requirements() {
    ...
    if [ ${#missing_commands[@]} -ne 0 ]; then
        colorize red "Missing required commands: ${missing_commands[*]}"
        colorize yellow "Attempting to install..."
        if command -v apt-get &> /dev/null; then
            sudo apt-get update && sudo apt-get install -y curl tar systemd openssl jq
        else
            colorize red "Unsupported package manager. Please install manually."
            exit 1
        fi
    fi
}
...
is_port_in_use() {
    local port=$1
    if sudo ss -tln | grep -q ":$port\s"; then return 0; else return 1; fi
}
...
generate_config_advanced() {
    ...
    if [ "$(echo "$params" | jq -r '.mode')" == "server" ]; then
        # Server Config generation logic... (same as v3.1)
        cat > "$config_file" << EOF
[server]
...
EOF
        ...
        # Client Config generation logic... (same as v3.1)
        cat > "$config_file" << EOF
[client]
...
configure_new_tunnel() {
    clear
    colorize cyan "--- Configure a New Tunnel ---"
...
configure_iran_server() {
    clear; colorize cyan "--- Configuring IRAN Server ---" "bold";
    ...
    read -p "[-] Accept UDP (true/false) [default: false]: " accept_udp;
    ...
    while true; do
        read -p "[-] Enter Web Port (default 0 to disable): " web_port; web_port=${web_port:-0};
    ...
    colorize cyan "[*] Supported Port Formats:";
    echo "1. 443-600"; echo "2. 443-600:5201"; echo "3. 443=600=1.1.1.1:5201";
    ...
    generate_config_advanced "$params"; create_service "$tunnel_name";
...
configure_kharej_server() {
    clear; colorize cyan "--- Configuring KHAREJ Server ---" "bold";
...
manage_tunnel() {
    clear; colorize cyan "--- Tunnel Management ---" "bold";
    ...
    read -p "Select tunnel to manage: " choice; selected_tunnel=${tunnels[$choice]};
...
check_tunnels_status() {
    clear; colorize cyan "--- Tunnels Status ---" "bold";
    if [ -z "$(ls -A $TUNNELS_DIR/*.toml 2>/dev/null)" ]; then
        colorize yellow "⚠️ No tunnels found."; press_key; return
    fi
    printf "%-20s %-15s %-22s\n" "NAME" "TYPE" "STATUS";
...
optimize_server() {
    ...
    # Optimization logic from Hawshemi script
...
display_logo_and_info() {
    # This is a combination of the display functions from rathole_v2.sh
    get_server_info
    
    colorize cyan "Script Version: v3.2 (Final Stable)"
    if [ -f "$BINARY_PATH" ]; then
        CORE_VERSION=$($BINARY_PATH --version 2>/dev/null | head -n 1)
        colorize cyan "Core Version: $CORE_VERSION"
        CORE_STATUS="$(colorize green Installed)"
    else
        colorize cyan "Core Version: N/A"
        CORE_STATUS="$(colorize red "Not Installed")"
    fi
    colorize cyan "Telegram Channel: @Gozar_Xray"
    echo -e "\e[93m═════════════════════════════════════════════\e[0m"
    colorize white "IP Address: $SERVER_IP"
    colorize white "Location: $SERVER_COUNTRY"
    colorize white "Datacenter: $SERVER_ISP"
    colorize white "Backhaul Core: $CORE_STATUS"
    echo -e "\e[93m═════════════════════════════════════════════\e[0m"
}
...
display_menu() {
    ...
    echo
    colorize white "1. Configure a new tunnel [IPv4/IPv6]"
    ...
    colorize white "5. Update & Install Backhaul Core"
    colorize white "6. Update & install script"
    colorize red   "7. Remove Backhaul"
...
read_option() {
    ...
    case $choice in
        ...
        0) exit 0 ;;
        *) colorize red "Invalid option. Please try again."; sleep 1 ;;
    esac
}
...
# The new, stable main loop from rathole_v2.sh
while true; do
    display_menu
    read_option
done

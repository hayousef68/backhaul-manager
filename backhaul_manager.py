...
class C:
    ...
    LOG_DIR, BINARY_PATH, TUNNELS_DIR = "/var/log/backhaul", f"/opt/backhaul/backhaul", f"/etc/backhaul/tunnels"
...
def get_core_version():
    if os.path.exists(C.BINARY_PATH):
        result = run_cmd([C.BINARY_PATH, '--version'])
...
[Service]
Type=simple
ExecStart={C.BINARY_PATH} -c {C.TUNNELS_DIR}/{tunnel_name}.toml
Restart=always
RestartSec=3
User=root
LimitNOFILE=1048576
...
"""
    service_path = f"{C.SERVICE_DIR}/{service_name}"
...
def create_monitor_timer_for_tunnel(tunnel_name):
    monitor_service = f"""[Unit]
Description=Monitor & Auto-Restart for Backhaul Tunnel: {tunnel_name}
After=network.target
...
if ! systemctl is-active --quiet backhaul-{tunnel_name}.service; then
  systemctl restart backhaul-{tunnel_name}.service
else
  journalctl -u backhaul-{tunnel_name}.service --since "3 min ago" --no-pager | grep -q "channel.*failed\\|connection.*failed\\|error" && systemctl restart backhaul-{tunnel_name}.service
fi
'
"""
    monitor_timer = f"""[Unit]
Description=Timer for monitoring backhaul tunnel: {tunnel_name}

[Timer]
OnBootSec=2min
OnUnitActiveSec=3min
Unit=backhaul-monitor-{tunnel_name}.service

[Install]
WantedBy=timers.target
"""
    service_path = f"/etc/systemd/system/backhaul-monitor-{tunnel_name}.service"
    timer_path = f"/etc/systemd/system/backhaul-monitor-{tunnel_name}.timer"
    with open("/tmp/backhaul-monitor.service", "w") as f:
        f.write(monitor_service)
    with open("/tmp/backhaul-monitor.timer", "w") as f:
        f.write(monitor_timer)
    run_cmd(['mv', '/tmp/backhaul-monitor.service', service_path], as_root=True)
    run_cmd(['mv', '/tmp/backhaul-monitor.timer', timer_path], as_root=True)
    run_cmd(['systemctl', 'daemon-reload'], as_root=True)
    run_cmd(['systemctl', 'enable', f'backhaul-monitor-{tunnel_name}.timer'], as_root=True)
    run_cmd(['systemctl', 'start', f'backhaul-monitor-{tunnel_name}.timer'], as_root=True)
...
def create_server_tunnel():
    ...
    run_cmd(['mv', f'/tmp/{tunnel_name}.toml', f"{C.TUNNELS_DIR}/{tunnel_name}.toml"], as_root=True)
    create_service(tunnel_name)
    run_cmd(['systemctl', 'start', f'backhaul-{tunnel_name}.service'], as_root=True)
    create_monitor_timer_for_tunnel(tunnel_name)
...
def create_client_tunnel():
    ...
    while True:
        ...
        if server_port and server_port.isdigit() and 1 <= int(server_port) <= 65535:
            break
        colorize("Valid port number is required (1-65535)!", C.RED)
    ...
    if test_connection.lower() == 'y':
        if shutil.which("nc") is None:
            colorize("netcat (nc) is not installed. Skipping test.", C.YELLOW)
        else:
...
def manage_tunnel():
    ...
    try:
        tunnel_files = [f for f in sorted(os.listdir(C.TUNNELS_DIR)) if f.endswith(".toml")]
        tunnels_info = []
        for filename in tunnel_files:
            tunnel_name = filename[:-5]
            config_path = os.path.join(C.TUNNELS_DIR, filename)
    ...
    while True:
        ...
        print("7) View Monitor Timer Status")
        print("8) Restart Monitor Timer")
        print("9) Remove Monitor Timer")
        print("\n0) Back")
        action = input("Choose an action: ")
        service_name = f"backhaul-{selected_tunnel}.service"
        monitor_timer = f"backhaul-monitor-{selected_tunnel}.timer"
        monitor_service = f"backhaul-monitor-{selected_tunnel}.service"
        if action == '6':
            ...
            if confirm == 'y':
                ...
                time.sleep(1)
                config_path = f"{C.TUNNELS_DIR}/{selected_tunnel}.toml"
                ...
                run_cmd(['rm', '-f', f"{C.SERVICE_DIR}/{service_name}"], as_root=True)
                # Remove monitor timer and service
                run_cmd(['systemctl', 'disable', '--now', monitor_timer], as_root=True)
                run_cmd(['rm', '-f', f"/etc/systemd/system/{monitor_timer}"], as_root=True)
                run_cmd(['rm', '-f', f"/etc/systemd/system/{monitor_service}"], as_root=True)
                run_cmd(['systemctl', 'daemon-reload'], as_root=True)
            ...
            clear_screen()
            run_cmd(['systemctl', 'status', monitor_timer], as_root=True, capture=False)
            press_key()
        elif action == '8':
            run_cmd(['systemctl', 'restart', monitor_timer], as_root=True)
            colorize("Monitor timer restarted.", C.GREEN)
            time.sleep(1)
        elif action == '9':
            run_cmd(['systemctl', 'disable', '--now', monitor_timer], as_root=True)
            run_cmd(['rm', '-f', f"/etc/systemd/system/{monitor_timer}"], as_root=True)
            run_cmd(['rm', '-f', f"/etc/systemd/system/{monitor_service}"], as_root=True)
            run_cmd(['systemctl', 'daemon-reload'], as_root=True)
            colorize("Monitor timer removed.", C.YELLOW)
            time.sleep(1)
...
def configure_new_tunnel():
    ...
    print(f"{C.BLUE}2) Create Kharej Client Tunnel (🌍){C.RESET}")
...
def install_backhaul_core():
    ...
    try:
        ...
        run_cmd(["mv", "/tmp/backhaul", C.BINARY_PATH], as_root=True)
        run_cmd(["chmod", "+x", C.BINARY_PATH], as_root=True)
...
def uninstall_backhaul():
    ...
    run_cmd(['pkill', '-f', C.BINARY_PATH], as_root=True)
    if os.path.exists(C.TUNNELS_DIR):
        tunnel_files = [f for f in os.listdir(C.TUNNELS_DIR) if f.endswith(".toml")]
        for filename in tunnel_files:
            tunnel_name = filename[:-5]
            service_name = f'backhaul-{tunnel_name}.service'
            monitor_timer = f"backhaul-monitor-{tunnel_name}.timer"
            monitor_service = f"backhaul-monitor-{tunnel_name}.service"
    ...
    run_cmd(['rm', '-rf', C.BACKHAUL_DIR, C.CONFIG_DIR, C.LOG_DIR], as_root=True)
    run_cmd(['systemctl', 'daemon-reload'], as_root=True)
...
def main():
    run_cmd(["mkdir", "-p", C.BACKHAUL_DIR, C.CONFIG_DIR, C.LOG_DIR, C.TUNNELS_DIR], as_root=True)
    if not os.path.exists(C.BINARY_PATH):
